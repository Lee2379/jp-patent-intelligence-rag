...
日本のAI関連特許を対象とした、完全ローカル・多言語対応の技術的先行技術探索支援システムです。
特許文書の構造を考慮した解析、日本語の語彙検索、多言語埋め込み、根拠に基づくローカル生成、
引用単位の原文確認、Human-in-the-loopレビュー、改ざん検知可能な監査証跡を統合しています。

> **本システムは技術的な先行技術探索を支援するものであり、法的助言を提供するものではありません。**
> クラウドアカウント、APIキー、有料API、従量課金サービスは不要です。
